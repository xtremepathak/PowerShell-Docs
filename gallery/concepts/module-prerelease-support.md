...
- Module versions displayed by Find-Module, Get-InstalledModule, and in the PowerShell Gallery will
  be displayed as a single string with the Prerelease string appended, as in 2.5.0-alpha.
...
- The Prerelease string may contain only ASCII alphanumerics [0-9A-Za-z-]. It is a best practice to
  begin the Prerelease string with an alpha character, as it will be easier to identify that this is
  a prerelease version when scanning a list of items.
- Only SemVer v1.0.0 prerelease strings are supported at this time. Prerelease string __must not__
...
installs 2.5.0-beta, the 2.5.0-beta version will __over-write__ the contents of the folder
