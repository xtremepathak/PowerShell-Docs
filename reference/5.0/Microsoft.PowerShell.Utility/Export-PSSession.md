...
### -AllowClobber
Indicates that this cmdlet exports the specified commands, even if they have the same names as commands in the current session.
...
### -ArgumentList
Specifies an array of the variant of the command that results from using the specified arguments.
...
### -CommandName
Specifies an array of commands with names or name patterns.
...
### -CommandType
Specifies that this cmdlet exports the specified types of command objects.
...
### -FormatTypeName
Specifies formatting instructions, as an array, only for the specified Microsoft .NET Framework types.
...
### -Module
Specifies as an array only the commands in the Windows PowerShell snap-ins and modules.
